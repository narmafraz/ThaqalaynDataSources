...
Usage:
    .venv/Scripts/python.exe app/raw/thaqalayn_api/scrape_thaqalayn_api.py
    # Or scrape specific books:
    .venv/Scripts/python.exe app/raw/thaqalayn_api/scrape_thaqalayn_api.py Fadail-al-Shia-Saduq
"""
...
# These are books from the aspiration list that we don't have raw data for,
# or that exist on thaqalayn.net but not in our 2020 mirror.
BOOKS_TO_SCRAPE = {
    # === The Four Books (3 missing) ===
    ...
    # === Additional valuable collections on API ===
...
def main():
    ...
    total_hadiths = sum(
        count for slug, (_folder, count) in BOOKS_TO_SCRAPE.items()
        if slug not in already_scraped
    )
    print("\nTotal hadiths to fetch: {}".format(total_hadiths))
...
if __name__ == "__main__":
    # Allow scraping specific books by passing slugs as arguments
    if len(sys.argv) > 1:
